...
class MyTrainData(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        corner_diff = np.load('filtered_corn_diff_mixed/{}.npz'.format(index))['arr_0']
        ls = corner_diff[corner_diff < 200]
        gt = ls[ls > -200]
        if (len(gt) != 8)
            dx_corner_diff = np.array([corner_diff[ev * 2 + 1] for ev in range(4)])
...
#                    print(inputs[0])
        ...
        if phase == 'val' and epoch_loss < best_loss:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(best_model_wts, "mixed_deep_hom_net_wts_odd")
            np.save("mixed_validation_corner_diff_output_odd", outputs.cpu().detach().numpy())
...
np.save("mixed_deep_hom_net_test_losses_odd", losses)
np.save("mixed_deep_hom_net_test_outputs_odd", model_outputs)
np.save("mixed_deep_hom_net_training_loss_odd", all_losses)